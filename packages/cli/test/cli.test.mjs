...
test('report --json, --by, plans and ROI', async () => {
  ...
  assert.equal(plan.monthly, 200);
  assert.deepEqual(plan.lines, [{ plan: 'max-20x', qty: 1, monthly: 200 }]);
  assert.ok(Math.abs(plan.api_equiv - claudeCost) < 1e-9);
  ...
  assert.match(text.stdout, /Plans:  claude  1× max-20x  = \$200\/mo  → API-equivalent \$[\d,.]+ +→ ROI \d+\.\d×/);
  assert.match(text.stdout, /\n {8}openai  1× pro {6}= \$200\/mo  → API-equivalent/);
  assert.match(text.stdout, /\n {8}total += \$400\/mo/);

...
- **Listed at $0 upstream = unpriced.** A LiteLLM or models.dev entry whose input and output rates are both 0 is not a price: every lookup step (exact, candidates, normalised match) skips it, so the model resolves through any other entry that does price it, and otherwise is `null` with the note `listed at $0 upstream` (shown like an override's unpriced note). An override with a real price still wins. `apps/web/scripts/seed-prices.mjs` applies the same rule (no row, and any stale row is deleted).
...
- `overrides.json`: `{ "<model>": { input, output, cache_read?, cache_write_5m?, cache_write_1h? } }` in USD per **million** tokens; missing cache fields fall back as for models.dev. An entry `{ "unpriced": "<note>" }` pins a model as unpriced (resolution stops; it is never priced by a fuzzy match) and the note is shown next to it in the report's unpriced list and in `--json` `unpriced[].note`. Only models with no public list price get such a note; never invent prices. A priced override is only for a vendor price change that LiteLLM/models.dev have not picked up yet: it must carry `source` (https URL of the vendor pricing page or announcement) and `added` (YYYY-MM-DD), both ignored by pricing, and is deleted once upstream matches (`pricing-diff.mjs` lists overrides that upstream now matches). See `docs/PRICING.md`. v1 ships `codex-auto-review` ("bundled reviewer, no list price") and `gpt-reserve` ("bundled with Codex, no list price"). models.dev: divide by 1e6; cache_write_1h = cache_write × 1.6 for anthropic (matches Anthropic's published 1h premium), = cache_write otherwise; missing cache_read → input; missing cache_write → input.
...
- Sanity test: for `claude-fable-5-1` with tokens (input 21721, output 513976, cache_write_5m 3570946, cache_read 221663704) cost must equal 125.97 ± 0.01 (matches ccusage) **at its 2026-09-22 LiteLLM rates, supplied as an inline fixture table** ($10 input, $0.25 cache read, $12.50 5m write, $20 1h write, $50 output per M), so the test checks the mapping and the math and a legitimate upstream price change never fails it. Same tokens with cache_write_1h instead of 5m must be higher, both on the fixture and on the bundled snapshot (a structural invariant of Anthropic pricing, not a price level). No test asserts a live snapshot price or a literal `SNAPSHOT_DATE`.
- `scripts/update-pricing.mjs` re-fetches both upstreams into the snapshots (dev/CI only, never at runtime). LiteLLM entries are kept for the providers the parsers can emit ids for, with every `*_cost*` field plus `litellm_provider`, `mode`, `max_input_tokens`, `max_output_tokens`, `deprecation_date`. An in-scope id that upstream drops (LiteLLM deletes retired models) is carried over from the bundled snapshot with `retained_since: <date>`, so historical usage stays priced. `SNAPSHOT_DATE` is bumped only when a snapshot file actually changes (it is the date the bundled prices last changed). `--check` exits 1 when upstream differs; `--out <dir>` writes the fresh snapshots elsewhere.
- `scripts/pricing-diff.mjs [base] [head]` (git refs or directories; default HEAD vs working tree) prints a markdown table of effective rate changes, additions and removals for the ids the parsers can emit; `--fail-on-change` exits 1 when there are any. The daily `.github/workflows/pricing.yml` runs the refresh and opens a PR with that table; see `docs/PRICING.md`.

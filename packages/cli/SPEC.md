...
tokenmaxxing link                         # prints URL; user opens it, pastes code back; stores token
...
tokenmaxxing schedule install|uninstall   # launchd (mac) / cron (linux) every 30 min running `sync`
...
- `buckets.jsonl` — append-only rows, one per (hour, source, model). Row schema:
  ...
  ```
  `sync` recomputes affected hours and appends a **replacement** row; the reader keeps the last row per key (ts,source,model). Periodically compact (rewrite keeping last per key) when file > 5MB.
...
- conversations: count `type=="user"` lines whose `message.content` is a string or contains a text block, deduped by `uuid`, excluding files under `/subagents/`.
- Hour bucket from `timestamp` (UTC, floor to hour).

### codex (`src/parsers/codex.ts`)
- Root: `${CODEX_HOME:-~/.codex}`. Files: `sessions/**/rollout-*.jsonl` and `archived_sessions/rollout-*.jsonl`.
...
- conversations: count `turn_context` lines with a new `root_turn_id`.
...
- Map: input←input − cached, cache_read←cached, output←output + tool, reasoning←thoughts (priced as output for Gemini). Dedup per file by message index (store count parsed per file). Model from message.model else `"gemini-unknown"`.
...
- LiteLLM mapping: input←input_cost_per_token, cache_read←cache_read_input_token_cost, cache_write_5m←cache_creation_input_token_cost, cache_write_1h←cache_creation_input_token_cost_above_1hr ?? cache_write_5m, output←output_cost_per_token. models.dev: divide by 1e6; cache_write_1h = cache_write × 1.6 for anthropic (matches Anthropic's published 1h premium), = cache_write otherwise; missing cache_read → input; missing cache_write → input.
...
If `ccusage` is on PATH: run `ccusage daily --since YYYYMMDD --json`, compare per-model input/output/cacheCreation/cacheRead totals with our claude buckets over the same UTC days, print a table with deltas and a pass/fail at 1% tolerance on tokens. Cost will differ (ccusage prices all cache writes at the 5m rate); print both and say why. If ccusage is missing, say how to install it and exit 0.

## link / push

- `link`: generate 8-char code, print `Open https://tokenmaxxing.fyi/link?code=XXXX and sign in`, then poll `GET {site}/api/v1/link/{code}` every 3s for up to 5 min; on `{ token, handle }` store both in config. (Site side is built separately; stub the poll with a clear error if 404.)
- `push`: send `POST {site}/api/v1/push` with header `Authorization: Bearer <token>` and body `{ v:1, deviceId, rows:[bucket rows since last pushed ts, ≤ 5000 per request] }`. Print the row count and the date range before sending. Store `lastPushedTs` in config. Never send paths, project names, prompts, or anything besides bucket rows.
...
- Print the exact diff before writing and require `--yes` or an interactive `y`.

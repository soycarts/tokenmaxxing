...
export default async function OrgLeaderboardPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [res, sponsor] = await Promise.all([getOrgLeaderboard(period), getSponsor("leaderboard:orgs")]);
  if (sponsor) after(() => recordImpression(sponsor, "leaderboard:orgs"));
